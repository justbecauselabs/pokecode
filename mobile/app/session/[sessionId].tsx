...
} from "../../src/components/session/MessageInput";
...
export default function SessionDetailScreen() {
	...
	const agentColors = agentsData?.agents?.reduce((acc, agent) => {
		if (agent.color) {
	...
	const [toolResult, setToolResult] =
		useState<AssistantMessageToolResult | null>(null);
		...
		return (
			<SafeAreaView className="bg-background">
				<View className="flex-1 justify-center items-center bg-background">
					<Text className="text-destructive text-lg font-mono">
						Invalid session ID
					</Text>
				</View>
			</SafeAreaView>
