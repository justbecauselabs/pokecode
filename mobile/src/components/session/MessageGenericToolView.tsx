...
export const MessageGenericToolView: React.FC<MessageGenericToolViewProps> = memo(
  ({ title, text, result, onResultPress }) => {
    return (
      <View className="p-3">
        ...
        <View className="flex-row items-center mb-2">
          ...
          <View className="ml-3 flex-1">
            {!result ? (
              ...
              onResultPress && (
                <TouchableOpacity onPress={() => onResultPress(result)} activeOpacity={0.7}>
        ...
        <View className="bg-gray-800 px-3 py-2 rounded mt-1">
          <Text className={`${textStyles.messageContentSm} text-gray-300 font-mono`}>{text}</Text>
        </View>
      </View>
    );
  }
