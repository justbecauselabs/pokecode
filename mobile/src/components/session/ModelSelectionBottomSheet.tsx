    ...
    >
      ...
      {error && (
        <View className="flex-1 items-center justify-center py-8 px-4">
          ...
          <Text className="text-destructive text-center font-medium">Failed to load models</Text>
          <Text className="text-muted-foreground text-center text-sm mt-1">{error.message}</Text>
        </View>
      )}
      ...
      {!isLoading && !error && models.length > 0 && (
        <View className="py-2 px-4">{models.map(renderModelItem)}</View>
      )}
    </CustomScrollableBottomSheet>
  );
});
