...
export const MessageToolView: React.FC<MessageToolViewProps> = memo(({ toolUse, toolResult, taskMessages, onResultPress, onTaskToolPress, agentColors }) => {
    ...
    switch (toolUse.type) {
      ...
      case 'glob':
        ...
        if ('pattern' in toolUse.data) {
          const pathText = 'path' in toolUse.data && toolUse.data.path 
            ? ` in ${toolUse.data.path}` 
            : '';
          return (
            <MessageGenericToolView
              title="glob"
