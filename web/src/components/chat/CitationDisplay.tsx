								...
								<div key={index} className="space-y-2">
									<div className="flex items-start gap-2">
										...
										<Badge variant="secondary" className="text-xs shrink-0">
											{citationGroup.length} ref
											{citationGroup.length !== 1 ? "s" : ""}
										</Badge>
									</div>
									...
									<div className="pl-5 space-y-1">
										{citationGroup.slice(0, 3).map((citation, citIndex) => (
											<div
												key={citIndex}
												className="text-xs bg-white/60 p-2 rounded border-l-2 border-amber-300"
											>
		...
		if (citationPos >= 0) {
			...
			elements.push(
				<span key={`citation-${index}`} className="relative">
					{citation.cited_text}
					<InlineCitation citationIndex={index} citation={citation} />
				</span>,
			);
