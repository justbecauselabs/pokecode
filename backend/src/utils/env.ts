...
export async function inferClaudeCodePath(): Promise<string> {
  try {
    ...
    const claudeCodePath = claudePath.replace(/\/claude$/, '/node_modules/@anthropic-ai/claude-code/cli.js');
    
    return claudeCodePath;
  } catch (error) {
    throw new Error(`Failed to infer CLAUDE_CODE_PATH: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
export async function getRepositoryPaths(): Promise<string[]> {
  try {
    ...
    console.warn(`Failed to read repositories from config: ${error instanceof Error ? error.message : 'Unknown error'}`);
    return [];
  }
}
