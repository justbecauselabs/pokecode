 ...
 */
export function parseJsonlMessage(
  rawData: unknown,
  lineIndex?: number,
  filePath?: string,
): JsonlMessage {
    ...
    logger.error(
      {
        ...
        rawDataKeys:
          typeof rawData === 'object' && rawData !== null ? Object.keys(rawData) : 'not-an-object',
      },
 ...
 * @throws {Error} If validation fails
 */
export function parseIntermediateMessage(rawData: unknown): IntermediateMessage {
  try {
 ...
 */
export function extractContent(msg: JsonlMessage): string {
  return extractContentFromValidatedMessage(msg);
}
 ...
 * @private
 */
function extractContentFromValidatedMessage(msg: JsonlMessage): string {
  if (msg.type === 'summary') {
    return msg.summary;
  }
 ...
 */
export function toIntermediateMessage(msg: JsonlMessage): IntermediateMessage {
  if (msg.type === 'summary') {
    return {
      id: msg.leafUuid,
      content: msg.summary,
  ...
  return {
    id: msg.uuid,
    content: extractContentFromValidatedMessage(msg),
